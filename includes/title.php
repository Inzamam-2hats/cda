...
function crb_get_title() {
	...
	if ( is_home() || ( is_single() && get_post_type() == 'post' ) ) {
		...
		$title = sprintf( __( 'Category Archives: <br>%s', 'sage' ), single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		$title = sprintf( __( 'Tag Archives: %s', 'sage' ), single_tag_title( '', false ) );
	} elseif ( is_day() ) {
		$title = sprintf( __( 'Daily Archives: %s', 'sage' ), get_the_time( 'F jS, Y' ) );
	} elseif ( is_month() ) {
		$title = sprintf( __( 'Monthly Archives: %s', 'sage' ), get_the_time( 'F, Y' ) );
	} elseif ( is_year() ) {
		$title = sprintf( __( 'Yearly Archives: %s', 'sage' ), get_the_time( 'Y' ) );
	} elseif ( is_author() ) {
		$title = sprintf( __( 'Posts by %s', 'sage' ), get_the_author() );
	} elseif ( is_archive() ) {
		$title = __( 'Archives', 'sage' );
	} elseif ( is_404() ) {
		$title = __( 'Error 404 - Not Found', 'sage' );
	} elseif ( is_page() ) {
		$title = get_the_title();
	}
